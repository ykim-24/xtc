...
contextBridge.exposeInMainWorld('electron', {
  ...
  // Claude chat
  claude: {
    send: (message: string, context: { activeFile?: { path: string; content: string }; contextFiles?: { path: string; content: string }[] }, projectPath: string | null) =>
      ipcRenderer.invoke('claude:send', message, context, projectPath),
