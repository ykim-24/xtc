...
contextBridge.exposeInMainWorld("electron", {
  ...
  revealInFinder: (path: string) =>
    ipcRenderer.invoke("file:revealInFinder", path),
  loadTypes: (projectPath: string) =>
    ipcRenderer.invoke("types:load", projectPath),
  ...
  onFileChanged: (
    ...
    const handler = (
      _event: Electron.IpcRendererEvent,
      data: { eventType: string; filename: string; dirPath: string }
    ) => callback(data);
  ...
  claude: {
    ...
    approveEdit: (editId: string, filePath?: string, content?: string) =>
      ipcRenderer.invoke("claude:approveEdit", editId, filePath, content),
    rejectEdit: (editId: string) =>
      ipcRenderer.invoke("claude:rejectEdit", editId),
    onPendingEditAdded: (callback: (edit: unknown) => void) => {
      const handler = (_event: Electron.IpcRendererEvent, edit: unknown) =>
        callback(edit);
      ipcRenderer.on("claude:pendingEditAdded", handler);
      return () =>
        ipcRenderer.removeListener("claude:pendingEditAdded", handler);
    },
    onPendingEditUpdated: (callback: (edit: unknown) => void) => {
      ...
      return () =>
        ipcRenderer.removeListener("claude:pendingEditUpdated", handler);
    },
    onActivity: (callback: (activity: string) => void) => {
      const handler = (_event: Electron.IpcRendererEvent, activity: string) =>
        callback(activity);
  ...
  // Test Runner
  detectTestFrameworks: (projectPath: string) =>
    ipcRenderer.invoke("tests:detectFrameworks", projectPath),
  detectTests: (projectPath: string, framework?: string) =>
    ipcRenderer.invoke("tests:detect", projectPath, framework),
  runTests: (
    projectPath: string,
    testFile?: string,
    testName?: string,
  ...
  lsp: {
    setProjectPath: (projectPath: string) =>
      ipcRenderer.invoke("lsp:setProjectPath", projectPath),
    startServer: (serverName: string) =>
      ipcRenderer.invoke("lsp:startServer", serverName),
    stopServer: (serverName: string) =>
      ipcRenderer.invoke("lsp:stopServer", serverName),
    stopAll: () => ipcRenderer.invoke("lsp:stopAll"),
    didOpen: (uri: string, language: string, content: string) =>
      ipcRenderer.invoke("lsp:didOpen", uri, language, content),
    didChange: (uri: string, language: string, content: string) =>
      ipcRenderer.invoke("lsp:didChange", uri, language, content),
    didClose: (uri: string, language: string) =>
      ipcRenderer.invoke("lsp:didClose", uri, language),
    getCompletions: (
      uri: string,
      language: string,
      line: number,
    ...
    formatDocument: (uri: string, language: string) =>
      ipcRenderer.invoke("lsp:formatDocument", uri, language),
    isServerAvailable: (language: string) =>
      ipcRenderer.invoke("lsp:isServerAvailable", language),
  ...
  linear: {
    ...
    getMyIssues: (apiKey: string) =>
      ipcRenderer.invoke("linear:getMyIssues", apiKey),
    getIssue: (apiKey: string, issueId: string) =>
      ipcRenderer.invoke("linear:getIssue", apiKey, issueId),
    createComment: (apiKey: string, issueId: string, body: string) =>
      ipcRenderer.invoke("linear:createComment", apiKey, issueId, body),
    deleteComment: (apiKey: string, commentId: string) =>
      ipcRenderer.invoke("linear:deleteComment", apiKey, commentId),
    getIssueStates: (apiKey: string, issueId: string) =>
      ipcRenderer.invoke("linear:getIssueStates", apiKey, issueId),
    updateIssue: (
      apiKey: string,
      issueId: string,
  ...
  git: {
    ...
    branch: (projectPath: string) =>
      ipcRenderer.invoke("git:branch", projectPath),
    remote: (projectPath: string) =>
      ipcRenderer.invoke("git:remote", projectPath),
    ...
    push: (projectPath: string, branch?: string) =>
      ipcRenderer.invoke("git:push", projectPath, branch),
    ...
    pr: {
      ...
      list: (projectPath: string) =>
        ipcRenderer.invoke("git:pr:list", projectPath),
      template: (projectPath: string) =>
        ipcRenderer.invoke("git:pr:template", projectPath),
      comments: (projectPath: string) =>
        ipcRenderer.invoke("git:pr:comments", projectPath),
      review: (
        projectPath: string,
        options: {
      ...
      onReviewProgress: (
        ...
        return () =>
          ipcRenderer.removeListener("git:pr:review:progress", handler);
      },
    },
    defaultBranch: (projectPath: string) =>
