...
app.whenReady().then(() => {
  ...
  if (isDev) {
    ...
    ipcMain.on("update:restart", () => {
      console.log(
        "[DEV] Restart requested - would quit and install in production"
      );
    });
  }
  ...
  function showTestUpdateNotification() {
    ...
    mainWindow.webContents.send("update-downloaded", {
      version: "0.2.0",
      releaseNotes:
        "<ul><li>Added welcome screen with wavy dot animation</li><li>Custom update notification UI</li><li>Auto-update support</li><li>Bug fixes and improvements</li></ul>",
    });
  ...
  try {
    ...
    for (const file of filesToUntrack) {
      try {
        ...
        const git = spawn("git", ["ls-files", "--error-unmatch", file], {
          cwd: projectPath,
          env: getEnvWithPath(),
        });
        ...
        if (isTracked) {
          ...
          const updateIndex = spawn(
            "git",
            ["update-index", "--skip-worktree", file],
            { cwd: projectPath, env: getEnvWithPath() }
          );
...
ipcMain.handle("file:watch", (_, dirPath: string) => {
  ...
  try {
    fileWatcher = watch(dirPath, { recursive: true }, (eventType, filename) => {
      if (
        filename &&
        !IGNORED_PATHS.some((ignored) => filename.includes(ignored))
      ) {
...
ipcMain.handle(
  "linear:getIssue",
  async (_, apiKey: string, issueId: string) => {
    try {
      const response = await fetch("https://api.linear.app/graphql", {
        method: "POST",
        headers: {
...
ipcMain.handle(
  ...
  async (_, apiKey: string, issueId: string, body: string) => {
    try {
      const response = await fetch("https://api.linear.app/graphql", {
        method: "POST",
        headers: {
...
ipcMain.handle(
  "linear:deleteComment",
  async (_, apiKey: string, commentId: string) => {
    try {
      const response = await fetch("https://api.linear.app/graphql", {
        method: "POST",
        headers: {
  ...
  async (_, apiKey: string, issueId: string) => {
    try {
      ...
      if (data.errors) {
        return {
          success: false,
          error: data.errors[0]?.message || "Failed to get states",
        };
      }
...
ipcMain.handle(
  ...
  async (
    ...
    if (
      ...
      summaryCache[cacheKey].commentCount === commentCount
    ) {
      return {
        success: true,
        summary: summaryCache[cacheKey].summary,
        cached: true,
      };
    }
  ...
  async (
    ...
    return new Promise(async (resolve) => {
      ...
      if (planOnly) {
        ...
        args.push(
          "--allowedTools",
          "Read,Glob,Grep,Task,WebFetch,WebSearch,TodoWrite,mcp__*"
        );
      } else {
            ...
            switch (event.type) {
                    ...
                    if (block.type === "tool_use") {
                      ...
                      const shortTarget =
                        typeof target === "string"
                          ? target.split("/").pop()?.substring(0, 25)
                          : "";

                      const newActivity = shortTarget
              ...
              case "user":
                ...
                if (event.tool_use_result) {
                  ...
                  if (result.file?.filePath) {
                    const fileName = result.file.filePath
                      .split("/")
                      .pop()
                      ?.substring(0, 25);
              ...
              case "result":
                ...
                if (event.usage || event.total_cost_usd !== undefined) {
                  ...
                  mainWindow?.webContents.send("claude:usage", {
                    projectPath,
                    usage,
                  });
                }
      ...
      claude.on("close", async (code) => {
        ...
        if (code === 0 || responseText.length > 0) {
          ...
          if (projectPath) {
            try {
              const newEdits = await detectChangesAndCreateEdits(
                projectPath,
                beforeSnapshot
              );
              if (newEdits.length > 0) {
                claudeLogger.info("Detected pending edits", {
                ...
                for (const newEdit of newEdits) {
                  ...
                  if (existingIndex !== -1) {
                    ...
                    claudeLogger.debug("Updated existing pending edit", {
                      filePath: newEdit.filePath,
                    });
                    mainWindow?.webContents.send(
                      "claude:pendingEditUpdated",
                      pendingEdits[existingIndex]
                    );
                  } else {
                    // Add new edit
...
ipcMain.handle(
  "claude:approveEdit",
  async (_, editId: string, filePath?: string, content?: string) => {
...
ipcMain.handle("terminal:create", (_, cwd?: string) => {
  ...
  terminalLogger.start("Creating terminal", {
    shell,
    cwd: cwd || os.homedir(),
  });
  ...
  const shellArgs = process.platform === "win32" ? [] : ["--login", "-i"]; // -l/--login for login shell, -i for interactive
  ...
  ptyProcess.onExit(({ exitCode }) => {
    terminalLogger.info("Terminal exited", {
      id: id.substring(0, 8),
      exitCode,
    });
...
ipcMain.handle(
  ...
  (_, id: string, cols: number, rows: number) => {
    const terminal = terminals.get(id);
    if (!terminal) {
      terminalLogger.warn("Terminal not found for resize", {
        id: id.substring(0, 8),
      });
      return { success: false, error: "Terminal not found" };
    }
    terminalLogger.debug("Resizing terminal", {
      id: id.substring(0, 8),
      cols,
      rows,
    });
...
ipcMain.handle("terminal:kill", (_, id: string) => {
  const terminal = terminals.get(id);
  if (!terminal) {
    terminalLogger.warn("Terminal not found for kill", {
      id: id.substring(0, 8),
    });
    return { success: false, error: "Terminal not found" };
  }
...
ipcMain.handle(
  "lsp:didOpen",
  async (_, uri: string, language: string, content: string) => {
    try {
...
ipcMain.handle(
  "lsp:getCompletions",
  async (_, uri: string, language: string, line: number, character: number) => {
    try {
...
ipcMain.handle(
  ...
  async (_, uri: string, language: string, line: number, character: number) => {
    try {
      const result = await lspManager.getReferences(
        uri,
        language,
        line,
        character
...
ipcMain.handle(
  "lsp:formatDocument",
  async (_, uri: string, language: string) => {
    try {
...
function parseJestOutput(
  ...
  try {
    ...
    for (const testResult of result.testResults || []) {
      ...
      const fileStatus = tests.some((t) => t.status === "failed")
        ? "failed"
...
function parsePlaywrightOutput(
  ...
  try {
    ...
    function extractSpecs(
      ...
      const tests: TestResult[] = [];
      const currentAncestors = suite.title
...
ipcMain.handle("tests:detectFrameworks", async (_, projectPath: string) => {
  ...
  testLogger.success("Detected frameworks", {
...
ipcMain.handle(
  "tests:detect",
  async (_, projectPath: string, framework?: string) => {
    ...
    try {
      ...
      const detectFileFramework = async (
        filePath: string
      ): Promise<
        "playwright" | "cypress" | "vitest" | "jest" | "mocha" | null
      > => {
        try {
          const content = await fs.readFile(filePath, "utf-8");
          ...
          if (
            content.includes('from "cypress"') ||
            content.includes("from 'cypress'") ||
            /\bcy\.(visit|get|contains|request)\s*\(/.test(content)
          ) {
            return "cypress";
          }
          if (
        ...
        try {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            while (
              ...
              bracketDepth <= describeStack[describeStack.length - 1].depth
            ) {
              describeStack.pop();
            }
            ...
            const testMatch = line.match(
              /^\s*(it|test|specify)(?:\.skip)?\s*\(\s*['"`](.+?)['"`]/
            );
            const skippedTestMatch = line.match(
              /^\s*(xit|xtest|it\.skip|test\.skip)\s*\(\s*['"`](.+?)['"`]/
            );
      ...
      testLogger.success("Detected test files", {
...
ipcMain.handle(
  ...
  async (
    ...
    if (!runner) {
      ...
      return {
        success: false,
        error: "No test runner detected. Install Jest, Vitest, or Playwright.",
      };
    }

    testLogger.start(`Running tests with ${runner}`, {
      file: testFile ? path.basename(testFile) : "all",
      testName,
    });
    ...
    return new Promise((resolve) => {
      ...
      proc.on("close", (_code) => {
        ...
        if (testFiles.length > 0) {
          ...
          const passed = testFiles.reduce(
            (sum, f) =>
              sum + f.tests.filter((t) => t.status === "passed").length,
            0
          );
          const failed = testFiles.reduce(
            (sum, f) =>
              sum + f.tests.filter((t) => t.status === "failed").length,
            0
          );
...
ipcMain.handle("git:branch", async (_, projectPath: string) => {
  ...
  const all = await runGit(projectPath, [
    "branch",
    "--format=%(refname:short)",
  ]);
  const remotes = await runGit(projectPath, [
    "branch",
    "-r",
    "--format=%(refname:short)",
  ]);
  ...
  gitLogger.info(`Current branch: ${currentBranch}`, {
    local: all.stdout.split("\n").filter(Boolean).length,
  });
...
ipcMain.handle("git:status", async (_, projectPath: string) => {
  ...
  const ahead = await runGit(projectPath, [
    "rev-list",
    "--count",
    "@{u}..HEAD",
  ]);
  const behind = await runGit(projectPath, [
    "rev-list",
    "--count",
    "HEAD..@{u}",
  ]);
  ...
  gitLogger.success(`Status loaded`, {
...
ipcMain.handle(
  "git:unstage",
  async (_, projectPath: string, files: string[]) => {
...
ipcMain.handle("git:push", async (_, projectPath: string, branch?: string) => {
  ...
  if (branch) {
    ...
    const trackingResult = await runGit(projectPath, [
      "rev-parse",
      "--abbrev-ref",
      "--symbolic-full-name",
      "@{u}",
    ...
    const hasUpstream =
      trackingResult.code === 0 && trackingResult.stdout?.trim();
    ...
    if (hasUpstream) {
      ...
      if (currentBranch) {
        gitLogger.info(
          `No upstream set, pushing with --set-upstream origin ${currentBranch}`
        );
        args = [
          "push",
          "--set-upstream",
  ...
  async (
    ...
    return new Promise((resolve) => {
      ...
      proc.on("error", () => {
        resolve({
          success: false,
          error: "GitHub CLI (gh) not found. Install it with: brew install gh",
        });
      });
    });
  }
...
ipcMain.handle("git:protectedBranches", async (_, projectPath: string) => {
  ...
  if (!owner || !repo) {
    return {
      success: false,
      error: "Could not parse GitHub remote",
      branches: [],
    };
  }
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      if (code !== 0) {
        ...
        resolve({
          success: false,
          error: stderr.trim() || "Failed to fetch branches",
          branches: [],
        });
        return;
      }
      ...
      try {
        ...
        console.error(
          "[git:protectedBranches] Parse error:",
          e,
          "stdout:",
          stdout
        );
...
ipcMain.handle("git:pr:list", async (_, projectPath: string) => {
  const proc = spawn(
    "gh",
    ["pr", "list", "--json", "number,title,state,headRefName,url"],
    {
      cwd: projectPath,
      env: getEnvWithPath(),
    }
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) {
        ...
        resolve({
          success: false,
          error: stderr.trim() || "Failed to list PRs",
        });
      }
    });
...
ipcMain.handle("git:pr:comments", async (_, projectPath: string) => {
  try {
    ...
    const prProc = spawn(
      "gh",
      ["pr", "view", branchName, "--json", "number,reviews,comments"],
      {
        cwd: projectPath,
        env: getEnvWithPath(),
      }
    ...
    const prResult = await new Promise<{
      success: boolean;
      data?: { number: number; reviews: unknown[]; comments: unknown[] };
      error?: string;
    }>((resolve) => {
      let stdout = "";
      let stderr = "";
      prProc.stdout.on("data", (data) => {
        stdout += data.toString();
      });
      prProc.stderr.on("data", (data) => {
        stderr += data.toString();
      });
      prProc.on("close", (code) => {
        if (code === 0) {
          ...
          resolve({
            success: false,
            error: stderr.trim() || "No PR found for this branch",
          });
        }
      });
      prProc.on("error", () =>
    ...
    const reviewCommentsProc = spawn(
      "gh",
      [
        "api",
        `repos/{owner}/{repo}/pulls/${prNumber}/comments`,
        "--jq",
        ".[] | {id, path, line, body, user: .user.login, createdAt: .created_at, side: .side, startLine: .start_line}",
      ],
      {
        cwd: projectPath,
        env: getEnvWithPath(),
    ...
    const reviewComments = await new Promise<
      Array<{
        id: number;
        path: string;
        line: number;
    ...
    >((resolve) => {
      let stdout = "";
      reviewCommentsProc.stdout.on("data", (data) => {
        stdout += data.toString();
      });
    ...
    const issueCommentsProc = spawn(
      "gh",
      [
        "api",
        `repos/{owner}/{repo}/issues/${prNumber}/comments`,
        "--jq",
        ".[] | {id, body, user: .user.login, createdAt: .created_at}",
      ],
      {
        cwd: projectPath,
        env: getEnvWithPath(),
    ...
    const issueComments = await new Promise<
      Array<{
        id: number;
        body: string;
        user: string;
    ...
    >((resolve) => {
      let stdout = "";
      issueCommentsProc.stdout.on("data", (data) => {
        stdout += data.toString();
      });
    ...
    return {
      success: false,
      error: err instanceof Error ? err.message : "Failed to fetch PR comments",
    };
  }
});
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      const prInfo = await new Promise<{ number: string; author: string }>(
        (resolve, reject) => {
          let stdout = "";
          let stderr = "";
          prInfoProc.stdout.on("data", (data) => {
            stdout += data.toString();
          });
          prInfoProc.stderr.on("data", (data) => {
            stderr += data.toString();
          });
          prInfoProc.on("close", (code) => {
            if (code === 0 && stdout.trim()) {
              ...
              reject(
                new Error(
                  stderr.trim() ||
                    "No PR found for current branch. Create a PR first."
                )
              );
            }
          });
          prInfoProc.on("error", () =>
            reject(new Error("GitHub CLI (gh) not found"))
          );
        }
      );
      ...
      if (isOwnPR && reviewAction === "REQUEST_CHANGES") {
        console.log(
          "[git:pr:review] Own PR detected, using COMMENT instead of REQUEST_CHANGES"
        );
      ...
      const repoFullName = await new Promise<string>((resolve, reject) => {
        let stdout = "";
        let stderr = "";
        repoProc.stdout.on("data", (data) => {
          stdout += data.toString();
        });
        repoProc.stderr.on("data", (data) => {
          stderr += data.toString();
        });
      ...
      const shaProc = spawn(
        "gh",
        ["pr", "view", prNumber, "--json", "headRefOid", "--jq", ".headRefOid"],
        {
          cwd: projectPath,
          env: getEnvWithPath(),
        }
      ...
      const commitSha = await new Promise<string>((resolve, reject) => {
        let stdout = "";
        let stderr = "";
        shaProc.stdout.on("data", (data) => {
          stdout += data.toString();
        });
        shaProc.stderr.on("data", (data) => {
          stderr += data.toString();
        });
      ...
      const prDiff = await new Promise<string>((resolve, reject) => {
        let stdout = "";
        let stderr = "";
        prDiffProc.stdout.on("data", (data) => {
          stdout += data.toString();
        });
        prDiffProc.stderr.on("data", (data) => {
          stderr += data.toString();
        });
      ...
      console.log(
        "[git:pr:review] Files in diff:",
        Array.from(validLinesByFile.keys())
      );
      ...
      const validComments = options.comments.filter((c) => {
        ...
        if (!fileLines) {
          console.log(
            `[git:pr:review] Skipping comment - file not in diff: ${c.path}`
          );
          return false;
        }
        if (!fileLines.has(c.line)) {
          console.log(
            `[git:pr:review] Skipping comment - line not in diff: ${c.path}:${c.line}`
          );
          return false;
        }
        return true;
      ...
      console.log(
        "[git:pr:review] Total comments:",
        options.comments.length,
        "Valid comments (in diff):",
        validComments.length
      );

      if (validComments.length === 0 && options.comments.length > 0) {
        console.log(
          "[git:pr:review] Warning: All comments filtered out. Comment paths and lines:",
      ...
      console.log(
        "[git:pr:review] First 3 comments:",
        JSON.stringify(formattedComments.slice(0, 3), null, 2)
      );
            ...
            if (code === 0) {
              ...
              try {
                ...
                if (response.comments && Array.isArray(response.comments)) {
                  commentIds = response.comments.map(
                    (c: { id: number }) => c.id
                  );
                }
              } catch {
      ...
      const rollback = async (commentIds: number[]) => {
        ...
        console.log(
          `[git:pr:review] Rolling back ${commentIds.length} comments...`
        );
      ...
      const sendProgress = (
        ...
        mainWindow?.webContents.send("git:pr:review:progress", {
          batch,
          total,
          status,
        });
      ...
      if (formattedComments.length <= BATCH_SIZE) {
        ...
        console.log(
          "[git:pr:review] Submitting single batch with",
        ...
        const payload = {
          ...
          event: reviewAction,
          comments:
            formattedComments.length > 0 ? formattedComments : undefined,
        };
      ...
      console.log(
        "[git:pr:review] Submitting",
        batches.length,
        "comment batches + 1 final review"
      );
      ...
      for (let i = 0; i < batches.length; i++) {
        const batchNum = i + 1;
        console.log(
          `[git:pr:review] Submitting batch ${batchNum}/${totalBatches} (${batches[i].length} comments)`
        );
        ...
        if (!result.success) {
          ...
          return {
            success: false,
            error: `Batch ${batchNum} failed: ${result.error}`,
          };
        }
      ...
      console.log(
        `[git:pr:review] Submitting final review (batch ${finalBatchNum}/${totalBatches}) with action: ${reviewAction}`
      );
      ...
      if (!finalResult.success) {
        ...
        return {
          success: false,
          error: `Final review failed: ${finalResult.error}`,
        };
      }
  ...
  try {
    ...
    const prInfoProc = spawn(
      ...
      [
        ...
        "--jq",
        "{number: .number, state: .state, branch: .headRefName}",
      ],
      {
        cwd: projectPath,
        env: getEnvWithPath(),
    ...
    const prInfo = await new Promise<{
      number: string;
      state: string;
      branch: string;
    }>((resolve, reject) => {
      ...
      prInfoProc.on("close", (code) => {
        if (code === 0 && stdout.trim()) {
          ...
          if (
            ...
            stderr.includes("no open pull requests")
          ) {
            reject(
              new Error(
                "No open PR found for current branch. The PR may have been merged or closed."
              )
            );
          } else {
            reject(
              new Error(stderr.trim() || "No PR found for current branch")
            );
          }
        }
      });
    ...
    if (prInfo.state !== "OPEN") {
      return {
        ...
        error: `PR #${
          prInfo.number
        } is ${prInfo.state.toLowerCase()}. Cannot review a ${prInfo.state.toLowerCase()} PR.`,
        files: [],
      };
    }
    ...
    const fullDiff = await new Promise<string>((resolve, reject) => {
      let stdout = "";
      let stderr = "";
      filesProc.stdout.on("data", (data) => {
        stdout += data.toString();
      });
      filesProc.stderr.on("data", (data) => {
        stderr += data.toString();
      });
...
ipcMain.handle(
  "git:diffFiles",
  async (_, projectPath: string, baseBranch?: string) => {
    ...
    if (!base) {
      ...
      for (const candidate of candidates) {
        const check = await runGit(projectPath, [
          "rev-parse",
          "--verify",
          candidate,
        ]);
    ...
    let compareRef = base;
    const mergeBaseResult = await runGit(projectPath, [
      "merge-base",
      base,
      "HEAD",
    ]);
    ...
    const filesResult = await runGit(projectPath, [
      "diff",
      "--name-only",
      compareRef,
    ]);
    ...
    for (const filePath of changedFiles) {
      const diffResult = await runGit(projectPath, [
        "diff",
        compareRef,
        "--",
        filePath,
...
ipcMain.handle(
  "git:worktree:saveDiff",
  async (_, worktreePath: string, sessionId: string, baseBranch?: string) => {
    // Determine base branch if not specified
    let base = baseBranch;
    if (!base) {
      const candidates = [
      ...
      for (const candidate of candidates) {
        const check = await runGit(worktreePath, [
          "rev-parse",
          "--verify",
          candidate,
        ]);
    ...
    let compareRef = base;
    const mergeBaseResult = await runGit(worktreePath, [
      "merge-base",
      base,
      "HEAD",
    ]);
...
ipcMain.handle(
  "git:worktree:readDiff",
  async (_, worktreePath: string, sessionId: string) => {
    const diffPath = path.join(
      worktreePath,
      ".xtc",
      `worktree-${sessionId}.patch`
    );
...
ipcMain.handle(
  "git:worktree:updateTokenUsage",
  async (
    _,
    worktreePath: string,
    sessionId: string,
    usage: Omit<TokenUsage, "lastUpdated" | "isClosed">
  ) => {
    const xtcDir = path.join(worktreePath, ".xtc");
    ...
    try {
      ...
      const updatedUsage: TokenUsage = {
        ...
        cacheWriteTokens:
          existingUsage.cacheWriteTokens + usage.cacheWriteTokens,
      ...
      await fs.writeFile(
        usagePath,
        JSON.stringify(updatedUsage, null, 2),
        "utf-8"
      );
...
ipcMain.handle(
  "git:worktree:readTokenUsage",
  async (_, worktreePath: string, sessionId: string) => {
    const usagePath = path.join(
      worktreePath,
      ".xtc",
      `token-usage-${sessionId}.json`
    );
      ...
      claude.on("close", (code) => {
        ...
        if (code === 0 && responseText) {
          try {
            ...
            resolve({
              success: true,
              review: { issues: [], summary: responseText },
            });
          }
        } else {
